...
def submit_prompt_flex(prompt, model="Qwen/Qwen3-30B-A3B-Instruct-2507", output_json=False, verbose=False):
    ...
    if endpoint == "anyscale":
        ...
        def generate(**kwargs):
            return client.messages.create(max_tokens=4000, **kwargs)

    else:
        # default: local vLLM(OpenAI-compatible)
        if verbose:
            print(f"Endpoint: local")
            print(f"Model: {model_fullname}")
        client = openai.OpenAI(
    ...
    if endpoint != "anthropic":
        return generated_output.choices[0].message.content
    return generated_output.content[0].text
